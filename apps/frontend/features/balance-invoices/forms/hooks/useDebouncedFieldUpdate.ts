...
type FieldName = "FUVrUnitario";

interface UseDebouncedFieldUpdateOptions<
...
> {
  ...
  delayMs?: number;
  fieldPath?: string; // Path to the field (e.g., "items.${index}.FUVrUnitario" or "facturau.${index}.FUVrUnitario")
}
...
>({
  ...
  delayMs = 600,
  fieldPath,
}: UseDebouncedFieldUpdateOptions<TFormData, T>) {
  ...
  // Use provided fieldPath or default to "items" (for ItemsFormData)
  const path = fieldPath || `items.${index}.${fieldName}`;

  // Watch the specific field value
  const currentValue = useWatch({
    control,
    name: path as Path<TFormData>,
  });
