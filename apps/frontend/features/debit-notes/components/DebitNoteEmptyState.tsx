...
interface DebitNoteEmptyStateProps {
  hasFilters: boolean;
  onClearFilters?: () => void;
}

export function DebitNoteEmptyState({
  hasFilters,
  onClearFilters,
}: DebitNoteEmptyStateProps) {
  if (hasFilters) {
    return (
      <div
  ...
  return (
    ...
    >
      <div className="flex size-12 items-center justify-center rounded-full bg-secondary">
        <NotebookText className="size-6 text-primary" aria-hidden />
      </div>
      <div className="space-y-1">
        <h3 className="text-base font-medium text-foreground">
          No debit notes yet
        ...
        <p className="max-w-sm text-sm text-muted-foreground">
          Debit notes registered against invoices will appear here.
        </p>
      </div>
    </div>
  );
