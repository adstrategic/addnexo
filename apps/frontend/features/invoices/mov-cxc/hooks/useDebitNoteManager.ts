...
import { useRegistrarNotaDebito } from "./useMovCXC";
import { handleMutationError } from "@/lib/errors/handle-error";
import { toast } from "sonner";
import { combineDateWithCurrentTimeUTC, toDateOnly } from "@/lib/dateUtils";

interface UseDebitNoteManagerOptions {
  onAfterSubmit?: () => void;
}

export function useDebitNoteManager(options?: UseDebitNoteManagerOptions) {
  // === UI STATE ===
  ...
  // === MUTATIONS ===
  const registrarNotaDebitoMutation = useRegistrarNotaDebito();

  // === FORM CONFIGURATION ===
  const defaultValues: DefaultValues<DebitNoteFormData> = {
    MCNroDocumento: "",
    MCDescripcion: "",
    MCFecha: new Date(),
  };
  ...
  const onSubmit = form.handleSubmit(
    async (data) => {
      ...
      try {
        ...
        await registrarNotaDebitoMutation.mutateAsync({
          invoiceId,
          data: dataToSend,
        });

        toast.success("Debit note registered successfully");
        close();
  ...
  return {
    ...
    form,
    onSubmit,
    isMutating: registrarNotaDebitoMutation.isPending,
  };
}
